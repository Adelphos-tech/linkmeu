...
import { Calendar, DollarSign, User, Mail, CheckCircle, Sparkles, MapPin, Tag } from 'lucide-react';
...
const MainPage = () => {
    ...
    useEffect(() => {
        fetch('/api/listings?status=approved&limit=8')
            .then(r => r.json())
            .then(data => {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-[#faf8f5] via-[#f5f0eb] to-[#ebe5dc]">
            ...
            {/* Listings Section */}
            <section className="relative max-w-7xl mx-auto px-6 lg:px-16 py-16">
                <div className="flex items-center gap-3 mb-10">
                    <div className="h-8 w-1 bg-gradient-to-b from-amber-500 to-orange-500 rounded-full"></div>
                    <h2 className="text-2xl lg:text-3xl font-bold text-gray-900">Featured Listings</h2>
                </div>
                ...
                {!loadingListings && listings.length === 0 && (
                    <div className="text-center py-12 bg-white/60 backdrop-blur-sm rounded-2xl border border-gray-100">
                        <p className="text-gray-500 text-lg">No approved listings yet. Be the first to post!</p>
                    </div>
                )}
                ...
                {!loadingListings && listings.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {listings.map((listing) => (
                            <div
                                key={listing.id}
                                className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-gray-900/5 transition-all duration-300 overflow-hidden cursor-pointer"
                                onClick={() => navigate(`/events`)}
                            >
                                <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
                                    <img
                                        src={listing.photos && listing.photos.length > 0 ? listing.photos[0] : 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=600&q=80'}
                                        alt={listing.title}
                                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                                        loading="lazy"
                                        onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=600&q=80'; }}
                                    />
                                    <span className="absolute top-3 left-3 inline-flex items-center gap-1.5 px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-semibold text-gray-700 capitalize shadow-sm">
                                        <Tag className="w-3 h-3 text-amber-600" />
                                        {listing.category}
                                    </span>
                                    <span className="absolute top-3 right-3 px-3 py-1 bg-gray-900/80 backdrop-blur-sm rounded-full text-xs font-medium text-white capitalize">
                                        {listing.purpose}
                                    </span>
                                </div>
                                <div className="p-5">
                                    <h3 className="font-semibold text-gray-900 mb-2 line-clamp-2 group-hover:text-amber-700 transition-colors">{listing.title}</h3>
                                    {listing.budget && (
                                        <p className="text-amber-700 font-bold text-sm mb-3">{listing.currency} {listing.budget}</p>
                                    )}
                                    <div className="flex items-center gap-1.5 text-gray-400 text-xs">
                                        <MapPin className="w-3.5 h-3.5" />
                                        <span className="truncate">{listing.location || listing.country || 'Singapore'}</span>
                                    </div>
                                    <div className="mt-3 pt-3 border-t border-gray-50 flex items-center justify-between">
                                        <span className="text-xs text-gray-400">{listing.seller_name}</span>
                                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${listing.status === 'approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                                            {listing.status}
                                        </span>
                                    </div>
                                </div>
